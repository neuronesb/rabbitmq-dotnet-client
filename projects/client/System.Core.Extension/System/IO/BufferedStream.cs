...
namespace System.IO
{
    ...
    private Task<Int32> _lastSyncCompletedReadTask;       // The last successful Task returned from ReadAsync
        ...
        public BufferedStream(Stream stream, Int32 bufferSize)
        {
            ...
            if (bufferSize <= 0)
                throw new ArgumentOutOfRangeException("bufferSize", bufferSize, "ArgumentOutOfRange_MustBePositive");

            Contract.EndContractBlock();

            Debug.WriteLine(!(stream is FileStream), "FileStream is buffered - don't wrap it in a BufferedStream");
            Debug.WriteLine(!(stream is MemoryStream), "MemoryStream shouldn't be wrapped in a BufferedStream!");
            Debug.WriteLine(!(stream is BufferedStream), "BufferedStream shouldn't be wrapped in another BufferedStream!");
    ...
    private static async Task FlushAsyncInternal(CancellationToken cancellationToken,
        ...
        private void ClearReadBufferBeforeWrite()
        {
            ...
            if (!_stream.CanSeek)
                throw new NotSupportedException("NotSupported_CannotWriteToBufferedStreamIfReadBufferCannotBeFlushed");
    ...
    private async Task WriteToUnderlyingStreamAsync(Byte[] array, Int32 offset, Int32 count,
        ...
        public override void SetLength(Int64 value)
        {
            ...
            if (value < 0)
                throw new ArgumentOutOfRangeException("value", "ArgumentOutOfRange_NegFileSize");
            Contract.EndContractBlock();
